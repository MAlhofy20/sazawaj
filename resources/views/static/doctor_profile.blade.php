...
@section('content')
    ...
    <section class="content">
        ...
        <div class="card" style="width:100% !important;">
            <!-- /.card-header -->
            <div class="card-body">
                <form action="{{route('site_post_doctor_profile')}}" id="profileForm" method="post" autocomplete="off" enctype="multipart/form-data">
                    {{csrf_field()}}
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-md-6">
                                <div class="form-group" style="position: relative;">
                                    <label for="field-2" class="control-label">{{Translate('مدة الانتظار بالدقائق')}}</label>
                                    <input type="text" autocomplete="nope" name="wait_time" value="{{Auth::user()->wait_time}}" id="wait_time" required class="form-control phone" placeholder="مدة الانتظار">
                                </div>
                            </div>
                            <div class="col-xs-12 col-md-12">
                                <div class="form-group">
                                    <label>{{Translate('نوع الخدمة')}}</label>
                                    <select name="service_type" id="service_type" class="form-control" onchange="changeServiceType()">
                                        <option value="none" {{Auth::user()->service_type == 'none' ? 'selected' : ''}}>{{Translate('بدون خدمات')}}</option>
                                        <option value="consulte" {{Auth::user()->service_type == 'consulte' ? 'selected' : ''}}>{{Translate('استشارة')}}</option>
                                        <option value="check" {{Auth::user()->service_type == 'check' ? 'selected' : ''}}>{{Translate('كشف')}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-xs-12 col-md-6 check" @if(Auth::user()->service_type != 'check') style="display:none" @endif>
                                <div class="form-group">
                                    ...
                                    <select name="payment_method" id="payment_method" class="form-control">
                                        ...
                                        <option value="cash"   {{Auth::user()->payment_method == 'cash' ? 'selected' : ''}}>{{Translate('في العيادة')}}</option>
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-md-12" @if(Auth::user()->service_type != 'check') style="display:none" @endif>
                                <h3>{{Translate('ايام الكشف')}}</h3>
                            </div>
                            @foreach (App\Models\Day::get() as $item)
                                <div class="col-xs-4 col-md-3 check" @if(Auth::user()->service_type != 'check') style="display:none" @endif>
                                    <div class="form-group">
...
@section('script')
    <script>
        ...
        function site_post_profile() {
            ...
            $.ajax({
                type        : 'POST',
                url         : '{{ route('site_post_doctor_profile') }}' ,
                datatype    : 'json' ,
                async       : false,
                processData : false,
                contentType : false,
