@extends('site.master')
@section('title') {{Translate('شركائنا')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{Translate('اضافة')}}" data-toggle="modal" data-target="#add-section">
                            <i class="fas fa-plus"></i>
                        </button>
                        <button class="btn btn-danger" onclick="deleteAllClient('more')" data-toggle="modal" data-target="#confirm-all-del">
                            <i class="fas fa-trash"></i>
                            {{Translate('حذف المحدد')}}
                        </button>
                        <button class="btn btn-danger" onclick="deleteAllClient('all')" data-toggle="modal" data-target="#confirm-all-del">
                            <i class="fas fa-trash"></i>
                            {{Translate('حذف الكل')}}
                        </button>
                    </div>
                    ...
                    <table id="datatable" class="table table-bordered table-striped">
                        <thead>
                            <tr style="text-align: center">
                                ...
                                {{-- <th>{{Translate('الاقسام الفرعية')}}</th> --}}
                        ...
                        <tbody>
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    ...
                                    {{-- <td>{{$item->title_ar}}</td> --}}
                                    {{-- <td><a href="{{ route('sub_sections' , $item->id) }}" class="btn btn-info">{{Translate('عرض')}} ({{$item->sub_sections->count()}})</a></td> --}}
                                    <td>{{ Carbon\Carbon::parse($item->created_at)->format('Y-m-d') }}</td>
                                    <td>
                                    <div class="btn-action">
                                        <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editClient({{ $item }})"  data-toggle="modal" data-target="#edit-section">
                                            <i class="fas fa-pen"></i>
                                        </a>

                                        <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deleteClient('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                            <i class="fas fa-trash"></i>
                                        </a>
                                    </div>
                                    </td>
...
@section('modal')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('إضافة شريك')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('addclient')}}" id="addClientForm" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label>{{Translate('الصورة')}}</label>
                    <div class="main-input">
                    <input class="file-input" type="file" name="photo" required accept="image/*"/>
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="addClient()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
    ...
    <div class="modal fade" id="edit-section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('تعديل شريك')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updateclient')}}" id="updateClientForm" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateClient()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">{{Translate('إغلاق')}}</button>
                </div>
            </form>
          </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form action="{{route('deleteclient')}}" method="post" class="d-flex align-items-center">
                @csrf
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form action="{{route('deleteclients')}}" method="post" class="d-flex align-items-center">
                @csrf
                <input type="hidden" name="client_ids" id="delete_ids">
    ...
    <script>
        $(function () {
            $(document).on("change", ".file-input", function () {
                ...
                uploadedImage.empty();
                uploadedImage.append('<img src="' + URL.createObjectURL(client.target.files[0]) + '">');
                }
            });
        ...
        function addClient() {
            ...
            $.ajax({
                ...
                processData : false,
                contentType : false,
                data        : new FormData($("#addClientForm")[0]),
        ...
        function updateClient() {
            ...
            $.ajax({
                ...
                processData : false,
                contentType : false,
                data        : new FormData($("#updateClientForm")[0]),
                success     : function(msg){
                    if(msg.value == '0'){
                        $('.save').notify(
                            msg.msg, {
