...
@section('title') {{Translate('البنوك')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{Translate('اضافة')}}" data-toggle="modal" data-target="#add-bank_account">
                        ...
                        <thead>
                            <tr style="text-align: center">
                                <th>
                                 ...
                                 <th>#</th>
                                {{--<th>{{Translate('صورة البنك')}}</th>--}}
                                <th>{{Translate('اسم البنك')}}</th>
                                {{--<th>{{Translate('اسم صاحب الحساب')}}</th>--}}
                                <th>{{Translate('رقم الحساب')}}</th>
                                <th>{{Translate('رقم الايبان')}}</th>
                                ...
                                <tr style="text-align: center">
                                    ...
                                    <td>
                                        <div class="btn-action">
                                            <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editBank_account({{ $item }})"  data-toggle="modal" data-target="#edit-bank_account">
                                                <i class="fas fa-pen"></i>
                                            </a>

                                            <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deleteBank_account('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </div>
                                    </td>
...
@section('modal')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('إضافة بنك')}}</h5>
          ...
          <div class="modal-body">
            <form action="{{route('addbank_account')}}" id="addBank_accountForm" method="POST" enctype="multipart/form-data">
                ...
                {{--<div class="form-group">
                    <label>{{Translate('صورة البنك')}}</label>
                ...
                <div class="form-group">
                    <label>{{Translate('اسم البنك ')}}</label>
                    <input type="text" name="title_ar" class="form-control">
                </div>
                {{--<div class="form-group">
                    <label>{{Translate('اسم البنك بالإنجليزية')}}</label>
                    <input type="text" name="title_en" class="form-control">
                </div>--}}
                {{--<div class="form-group">
                    <label>{{Translate('اسم صاحب الحساب')}}</label>
                    <input type="text" name="account_name" class="form-control">
                </div>--}}
                <div class="form-group">
                    <label>{{Translate('رقم الحساب')}}</label>
                    <input type="text" name="account_number" id="" class="form-control phone">
                </div>
                <div class="form-group">
                    <label>{{Translate('رقم الايبان')}}</label>
                    <input type="text" name="iban_number" class="form-control">
                </div>
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="addBank_account()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
    ...
    <div class="modal fade" id="edit-bank_account" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('تعديل بنك')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updatebank_account')}}" id="updateBank_accountForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>{{Translate('رقم الحساب')}}</label>
                    <input type="text" name="account_number" id="edit_account_number" class="form-control phone">
                </div>
                <div class="form-group">
                    <label>{{Translate('رقم الايبان')}}</label>
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateBank_account()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">{{Translate('إغلاق')}}</button>
                </div>
            </form>
          </div>
