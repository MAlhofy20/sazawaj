                ...
                @if($type != 'try')
                    <div class="col-12">
                        <div class="btns header-buttons">
                            @if($type != 'static_slider')
                                <button class="btn bg-teal" title="اضافة" data-toggle="modal" data-target="#add-slider">
                        ...
                        <tbody>
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    ...
                                    {{-- @if($type == 'app')
                                        <td>{{ slider_type($item->sub_type) }}</td>
                                    @endif --}}
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-action">
                                            <a href="#" class="btn btn-sm bg-teal" title="تعديل" onclick="editSection({{ $item }})"  data-toggle="modal" data-target="#edit-slider">
                                                <i class="fas fa-pen"></i>
                                            </a>

                                            @if($type != 'try' && $type != 'static_slider')
                                                <a href="#" class="btn btn-sm btn-danger" title="حذف" onclick="deleteSection('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                                    <i class="fas fa-trash"></i>
                                                </a>
                                            @endif
                                        </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{route('addslider')}}" id="addSectionForm" method="POST" enctype="multipart/form-data">
                ...
                {{-- @if($type == 'app')
                    <div class="form-group">
                        <label>مكان ظهور الاعلان</label>
                        <select name="sub_type" class="form-control">
                            <option value="home">الرئيسية</option>
                            <option value="market">السوق الخيري</option>
                            <option value="volunteer">التبرع</option>
                            <option value="services">خدمتنا</option>
                        </select>
                    </div>
                @endif --}}
                {{--@if($type == 'app')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="desc_en" class="form-control" dir="ltr" rows="3"></textarea>
                    </div>
                @endif--}}
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="addSection()" class="btn btn-sm btn-success save">حفظ</button>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{route('updateslider')}}" id="updateSectionForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateSection()" class="btn btn-sm btn-success save">حفظ</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">إغلاق</button>
                </div>
            </form>
          </div>
...
@section('script')
    <script>
        ...
        function editSection(slider){
            ...
            $('#edit_sub_type').val(slider.sub_type);
            $('#edit_url').val(slider.url);
            $('#edit_desc_ar').val(slider.desc_ar);
            $('#edit_desc_en').val(slider.desc_en);
