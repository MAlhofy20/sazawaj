    ...
    <section class="content">
        ...
        <div class="card" style="width:100% !important;">
            <div class="card-header">
                ...
                <div class="col-12">
                    <div class="btns header-buttons text-center">
                        ...
                        <a class="btn btn-success waves-effect btn-lg waves-light viewData"
                            ...
                            @if ($user->user_type == 'provider') اجمالي مكسب المندوب :
                            @else اجمالي مكسب التاجر : @endif
                            <span>{{ (float) $user->total_user_benfit }}</span>
                        </a>
                        <a class="btn btn-success waves-effect btn-lg waves-light viewData"
                            style="width: 350px;margin: 2px;">المتبقي المندوب : <span
                                id="pay">{{ (float) $user->total_user_debt }}</span></a>
                        <a class="btn btn-success waves-effect btn-lg waves-light viewData"
                            style="width: 350px;margin: 2px;">اجمالي مكسب التطبيق :
                            <span>{{ (float) $user->total_admin_benfit }}</span></a>
                        <a class="btn btn-success waves-effect btn-lg waves-light viewData"
                            style="width: 350px;margin: 2px;">المتبقي للتطبيق : <span
                                id="debt">{{ (float) $user->total_admin_debt }}</span></a>
                    </div>
                </div>
                <div class="col-12">
                    <hr>
                ...
                <div class="col-12">
                    <div class="btns header-buttons">
                        @if ($user->user_type == 'provider')
                            <form action="{{ route('providerpaid') }}" method="post">
                            @elseif($user->user_type == 'market')
                                <form action="{{ route('marketpaid') }}" method="post">
                                @elseif($user->user_type == 'saler')
                                    <form action="{{ route('salerpaid') }}" method="post">
                        @endif
                        @csrf
                ...
                <div class="col-12">
                    <h3>
                    @if ($user->user_type == 'provider') سداد حساب المندوب : @else سداد
                            حساب للتاجر : @endif
                    </h3>
                </div>
                <div class="col-12">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="datatable-button" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach (App\Models\User_transction::whereUserId($user->id)->orderBy('id', 'desc')->get() as $i => $item)
                                <tr style="text-align: center">
                                    <td>{{ ++$i }}</td>
                                    <td>{{ $item->amount }}</td>
                                    <td>{{ Carbon\Carbon::parse($item->created_at)->format('Y-m-d') }}</td>
