...
@section('title') {{ Translate('الصفحات') }} @endsection
@section('style')
            ...
            {{-- <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{Translate('اضافة')}}" data-toggle="modal" data-target="#add-page">
                                ...
                                <tr style="text-align: center">
                                    ...
                                    <td>
                                        <div class="btn-action">
                                            <a href="{{ url('edit-page-page/' . $item->id) }}"
                                               class="btn btn-sm bg-teal" title="{{ Translate('تعديل') }}"
                                               {{--onclick="editpage({{ $item }})" data-toggle="modal" data-target="#edit-page"--}} >
                                                <i class="fas fa-pen"></i>
                                            </a>

                                            {{-- <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deletepage('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                            <i class="fas fa-trash"></i>
                                        </a> --}}
                                        </div>
                                    </td>
...
@section('modal')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ Translate('إضافة صفحة') }}</h5>
                ...
                <div class="modal-body">
                    <form action="{{ route('addpage') }}" id="addpageForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="desc_en" dir="ltr" class="form-control" rows="5"></textarea>
                        </div>
    ...
    <div class="modal fade" id="edit-page" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ Translate('تعديل صفحة') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('updatepage') }}" id="updatepageForm" method="POST"
                        ...
                        <div class="form-group">
                            <label>{{ Translate('العنوان بالإنجليزية') }}</label>
                            <input type="text" name="title_en" id="edit_title_en" dir="ltr" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>{{ Translate('التفاصيل الكاملة بالعربية') }}</label>
                        ...
                        <div class="form-group">
                            <label>{{ Translate('التفاصيل الكاملة بالإنجليزية') }}</label>
                            <textarea name="desc_en" id="edit_desc_en" dir="ltr" class="form-control" rows="5"></textarea>
                        </div>
                        <div class="d-flex justify-content-center">
                            <button type="submit" onclick="updatepage()"
                                class="btn btn-sm btn-success save">{{ Translate('حفظ') }}</button>
                            <button class="btn btn-sm btn-danger" data-dismiss="modal">{{ Translate('إغلاق') }}</button>
                        </div>
                    </form>
...
@section('script')
    <script>
        ...
        function editpage(page) {
            ...
            $('#edit_desc_ar').val(page.desc_ar);
            $('#edit_desc_en').val(page.desc_en);
            $('#edit_image').removeAttr('src');
            $('#edit_image').attr('src', "{{ url('') }}" + page.image);
        }
