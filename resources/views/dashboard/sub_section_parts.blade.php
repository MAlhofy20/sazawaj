...
@section('title') {{Translate('فصول الكتاب')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{Translate('اضافة')}}" data-toggle="modal" data-target="#add-sub_section_part">
                    ...
                    <table id="datatable" class="table table-bordered table-striped">
                        ...
                        @foreach ($data as $i=>$item)
                            <tr style="text-align: center">
                                ...
                                <td>{{$item->title_ar}}</td>
                                <td>{{is_null($item->sub_section) ? '' : $item->sub_section->title}}</td>
                                <td>{{ Carbon\Carbon::parse($item->created_at)->format('Y-m-d') }}</td>
                                <td>
                                    <div class="btn-action">
                                        <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editSection({{ $item }})"  data-toggle="modal" data-target="#edit-sub_section_part">
...
@section('modal')
    <!-- add-sub_section_part modal-->
    <div class="modal fade" id="add-sub_section_part" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('addsub_section_part')}}" id="addSectionForm" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="sub_section_id" id="sub_section_id" value="{{$sub_section_id}}">
                        ...
                        <div class="form-group" @if(!$seen) style="display: none;" @endif>
                            <label>{{Translate('قسم')}}</label>
                            <select name="type" id="" class="form-control" required>
                                <option value="student">{{Translate('قسم الطالب')}}</option>
                                <option value="exam">{{Translate('قسم النشاط')}}</option>
                            </select>
                        </div>
    ...
    <!--end add-sub_section_part modal-->
    <!-- edit-sub_section_part modal-->
    <div class="modal fade" id="edit-sub_section_part" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="{{route('updatesub_section_part')}}" id="updateSectionForm" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    <!--end edit-sub_section_part modal-->
    <!-- confirm-del modal-->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{route('deletesub_section_part')}}" method="post" class="d-flex align-items-center">
                        @csrf
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{route('deletesub_section_parts')}}" method="post" class="d-flex align-items-center">
                        @csrf
                        <input type="hidden" name="sub_section_part_ids" id="delete_ids">
...
@section('script')
    <script>
        ...
        function editSection(sub_section_part){
            $('#edit_id').val(sub_section_part.id);
            $('#edit_section_id').val(sub_section_part.section_id);
            $('#type').val(sub_section_part.type);
            $('#edit_title_ar').val(sub_section_part.title_ar);
            $('#edit_title_en').val(sub_section_part.title_en);
            $('#edit_image').removeAttr('src');
            $('#edit_image').attr('src' , "{{url('')}}"+sub_section_part.image);
        }
        ...
        function updateSection() {
            ...
            $.ajax({
                type        : 'POST',
                url         : '{{ route('updatesub_section_part') }}' ,
                datatype    : 'json' ,
                async       : false,
                processData : false,
                contentType : false,
        ...
        function checkIds(){
            let sub_section_partsIds = '';
            $('.checkSingle:checked').each(function () {
                let id = $(this).attr('id');
                sub_section_partsIds += id + ' ';
            });
            let requestData = sub_section_partsIds.split(' ');
