@extends('site.master')
@section('title') {{Translate('المدن')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{Translate('اضافة')}}" data-toggle="modal" data-target="#add-city">
                            ...
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    ...
                                    <div class="btn-action">
                                        <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editCity({{ $item }})"  data-toggle="modal" data-target="#edit-city">
                                            <i class="fas fa-pen"></i>
                                        </a>

                                        <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deleteCity('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                            <i class="fas fa-trash"></i>
                                        </a>
                                    </div>
                                    </td>
...
@section('modal')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('إضافة مدينة')}}</h5>
          ...
          <div class="modal-body">
            <form action="{{route('addcity')}}" id="addCityForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="addCity()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
    ...
    <div class="modal fade" id="edit-city" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('تعديل مدينة')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updatecity')}}" id="updateCityForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateCity()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">{{Translate('إغلاق')}}</button>
                </div>
            </form>
          </div>
