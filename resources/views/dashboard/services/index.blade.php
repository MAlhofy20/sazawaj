            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <a href="{{ route('addpageservice', $sub_section_id) }}" class="btn bg-teal">
                                    ...
                                    <td>{{ $item->title_ar }}</td>
                                     <td>
                                        <div
                                            ...
                                            <input type="checkbox" onchange="changeUserIsFav('{{ $item->id }}')"
                                                {{ !$item->is_fav ? '' : 'checked' }} class="custom-control-input"
                                                id="is_fav_customSwitch{{ $item->id }}">
                                            <label class="custom-control-label" id="is_fav_status_label{{ $item->id }}"
                                                for="is_fav_customSwitch{{ $item->id }}">{{ !$item->is_fav ? 'اخفاء' : 'اظهار' }}</label>
                                        </div>
                                    </td>
                                    {{-- <td>
                                        <div
                                            class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                                            <input type="checkbox" onchange="changeUserBest('{{ $item->id }}')"
                                                {{ !$item->best ? '' : 'checked' }} class="custom-control-input"
                                                id="best_customSwitch{{ $item->id }}">
                                            <label class="custom-control-label" id="best_status_label{{ $item->id }}"
                                                for="best_customSwitch{{ $item->id }}">{{ !$item->best ? 'اخفاء' : 'اظهار' }}</label>
                                        </div>
                                    </td> --}}
                                    <td>
                                        <div
                                            ...
                                            <input type="checkbox" onchange="changeUserStatus('{{ $item->id }}')"
                                                {{ !$item->show ? '' : 'checked' }} class="custom-control-input"
                                                id="customSwitch{{ $item->id }}">
                                            <label class="custom-control-label" id="status_label{{ $item->id }}"
                                                for="customSwitch{{ $item->id }}">{{ !$item->show ? 'اخفاء' : 'اظهار' }}</label>
                                        </div>
                                    </td>
                                    <td>{{ Carbon\Carbon::parse($item->created_at)->format('Y-m-d') }}</td>
                                    <td>
                                        <div class="btn-action">
                                            <a href="{{ route('editpageservice', $item->id) }}"
                ...
                <div class="modal-body">
                    <form action="{{ route('addservice') }}" id="addServiceForm" method="POST"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="price" id="" class="form-control phone"
                                placeholder="سعر (الكيلو, اللتر, ....)">
                        </div>
                        <div class="form-group">
                            <label>السعرات الحرارية</label>
                            <input type="text" name="calories" id="" class="form-control phone"
                                placeholder="السعرات الحرارية">
                        </div>
                        <div class="form-group">
                            <label>القيمة المضافة</label>
                            <div class="input-group">
                                <input type="tel" name="value_added" value="15" class="form-control phone">
                        ...
                        <div class="form-group">
                            <label>التفاصيل الكاملة بالإنجليزية</label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route('deleteservices') }}" method="post" class="d-flex align-items-center">
                        @csrf
                        <input type="hidden" name="service_ids" id="delete_ids">
...
@section('script')
    <script>
        ...
        function changeUserIsFav(id) {
            var tokenv  = "{{csrf_token()}}";
            $.ajax({
                type     : 'POST',
                url      : '{{ route('changeis_favservice') }}' ,
                datatype : 'json' ,
                data     : {
                    'id'         :  id ,
                    '_token'     :  tokenv
                    ...
                    if(msg == 0)
                        ...
                        $('#is_fav_status_label'+id).html(msg);
                }
            });
        }

        function changeUserBest(id) {
            var tokenv  = "{{csrf_token()}}";
            $.ajax({
                type     : 'POST',
                url      : '{{ route('changebestservice') }}' ,
                datatype : 'json' ,
                data     : {
                    'id'         :  id ,
                    '_token'     :  tokenv
