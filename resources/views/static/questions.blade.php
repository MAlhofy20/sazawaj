@extends('site.master')
@section('title') {{Translate('الاسئلة الشائعة')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        ...
                        <button class="btn btn-danger" onclick="deleteAllQuestion('more')" data-toggle="modal" data-target="#confirm-all-del">
                            <i class="fas fa-trash"></i>
                            {{Translate('حذف المحدد')}}
                        </button>
                        <button class="btn btn-danger" onclick="deleteAllQuestion('all')" data-toggle="modal" data-target="#confirm-all-del">
                            <i class="fas fa-trash"></i>
                            {{Translate('حذف الكل')}}
                        </button>
                    </div>
                            ...
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    ...
                                    <div class="btn-action">
                                        <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editQuestion({{ $item }})"  data-toggle="modal" data-target="#edit-section">
                                            <i class="fas fa-pen"></i>
                                        </a>

                                        <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deleteQuestion('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                            <i class="fas fa-trash"></i>
                                        </a>
                                    </div>
                                    </td>
...
@section('modal')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('إضافة سؤال')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('addquestion')}}" id="addQuestionForm" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    <label>{{Translate('السؤال بالعربية')}}</label>
                    <input type="text" name="title_ar" class="form-control">
                </div>
                <div class="form-group">
                    <label>{{Translate('السؤال بالإنجليزية')}}</label>
                ...
                <div class="form-group">
                    <label>{{Translate('الاجابة بالعربية')}}</label>
                    <textarea name="desc_ar" class="form-control" rows="3"></textarea>
                </div>
                <div class="form-group">
                    <label>{{Translate('الاجابة بالإنجليزية')}}</label>
                    <textarea name="desc_en" class="form-control" rows="3"></textarea>
                </div>

                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="addQuestion()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
    ...
    <div class="modal fade" id="edit-section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('تعديل سؤال')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updatequestion')}}" id="updateQuestionForm" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateQuestion()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">{{Translate('إغلاق')}}</button>
                </div>
            </form>
          </div>
    ...
    <script>
        ...
        function addQuestion() {
            ...
            $.ajax({
                ...
                processData : false,
                contentType : false,
                data        : new FormData($("#addQuestionForm")[0]),
        ...
        function updateQuestion() {
            ...
            $.ajax({
                ...
                processData : false,
                contentType : false,
                data        : new FormData($("#updateQuestionForm")[0]),
                success     : function(msg){
                    if(msg.value == '0'){
                        $('.save').notify(
                            msg.msg, {
        ...
        function deleteAllQuestion(type){
