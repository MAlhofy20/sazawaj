...
class salerController extends Controller
{
    ...
    public function index()
    {
        ...
        //$data = get_users_by('saler', 'asc', 10);
        //$data = User::where('user_type', 'saler')->orderBy('id', 'desc')->paginate(20);
        return view('dashboard.salers', compact('data' , 'queries'));
    }
    ...
    public function paid(Request $request)
    {
        ...
        #add adminReport
        admin_report('تسوية حساب الطبيب ' . $user->name);
    ...
    public function store(Request $request)
    {
        ...
        #add adminReport
        admin_report('أضافة الطبيب ' . $user->name);
    ...
    public function edit($id)
    {
        ...
        return view('dashboard.salers_edit', compact('data'));
    }
    ...
    public function update(Request $request)
    {
        ...
        #add adminReport
        admin_report('تعديل الطبيب ' . $user->name);
    ...
    public function delete_all(Request $request)
    {
        $type = $request->type;
        #get salers
        if ($type == 'all') $users = User::where('user_type', 'saler')->get();
        else {
    ...
    #change saler status (active - blocked)
    public function confirm_saler(Request $request)
    {
        //check data
        $user = User::whereId($request->id)->first();
        ...
        //add report
        admin_report(' تفعيل الطبيب ' . $user->name);
        $msg = Translate('حساب مفعل');
        return $msg;
    }

    #change saler status (active - blocked)
    public function change_saler_status(Request $request)
    {
        //check data
        $user = User::whereId($request->id)->first();
        ...
        //add report
        $user->blocked ? admin_report(' حظر الطبيب ' . $user->name) : admin_report('ألغاء حظر الطبيب ' . $user->name);
        $msg = $user->blocked ? Translate('حظر') : Translate('مفعل');
        return $msg;
    }

    #change saler fav
    public function change_saler_fav(Request $request)
    {
        //check data
        $user = User::whereId($request->id)->first();
        if (!isset($user)) return 0;
        ...
        //add report
        !$user->is_fav ? admin_report(' الغاء ظهور الطبيب ' . $user->name) : admin_report('بظهور الطبيب ' . $user->name);
    ...
    public function send_notify(Request $request)
    {
        ...
        if ($request->type == 'all') $users = User::where('user_type', 'saler')->get();
