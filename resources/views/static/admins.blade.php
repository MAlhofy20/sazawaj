@extends('site.master')
@section('title') {{Translate('المشرفين')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{Translate('اضافة')}}" data-toggle="modal" data-target="#add-admin">
                        ...
                        <tbody>
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    <td>
                                        <label class="custom-control material-checkbox" style="margin: auto">
                                            <input type="checkbox" class="material-control-input checkSingle" id="{{$item->id}}" @if($item->id == 1) disabled @endif>
                                            <span class="material-control-indicator"></span>
                                        </label>
                                    </td>
                                    {{-- <td>{{ ++$i }}</td> --}}
                                    ...
                                    <div class="btn-action">
                                            {{-- <a href="#" class="btn btn-sm btn-primary" title="{{Translate('عرض')}}" onclick="showAdmin({{ $item }})"  data-toggle="modal" data-target="#user-profile">
                                                <i class="fas fa-user"></i>
                                            </a> --}}

                                            <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editAdmin({{ $item }})"  data-toggle="modal" data-target="#edit-admin">
                                                <i class="fas fa-pen"></i>
                                            </a>

                                            @if($item->id != 1)
                                                <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deleteAdmin('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                                    <i class="fas fa-trash"></i>
                                                </a>
                                            @endif
                                    </div>
...
@section('modal')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('إضافة مشرف')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('addadmin')}}" id="addAdminForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>{{Translate('الصلاحية')}}</label>
    ...
    <div class="modal fade" id="edit-admin" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('تعديل مشرف')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updateadmin')}}" id="updateAdminForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateAdmin()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">{{Translate('إغلاق')}}</button>
                </div>
            </form>
          </div>
