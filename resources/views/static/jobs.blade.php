@extends('site.master')
@section('title') {{Translate('الوظائف')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        ...
                        <button class="btn btn-danger" onclick="deleteAllJob('more')" data-toggle="modal" data-target="#confirm-all-del">
                            <i class="fas fa-trash"></i>
                            {{Translate('حذف المحدد')}}
                        </button>
                        <button class="btn btn-danger" onclick="deleteAllJob('all')" data-toggle="modal" data-target="#confirm-all-del">
                            <i class="fas fa-trash"></i>
                            {{Translate('حذف الكل')}}
                        </button>
                    </div>
                            ...
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    ...
                                    <div class="btn-action">
                                        <a href="#" class="btn btn-sm bg-teal" title="{{Translate('تعديل')}}" onclick="editJob({{ $item }})"  data-toggle="modal" data-target="#edit-section">
                                            <i class="fas fa-pen"></i>
                                        </a>

                                        <a href="#" class="btn btn-sm btn-danger" title="{{Translate('حذف')}}" onclick="deleteJob('{{ $item->id }}')"  data-toggle="modal" data-target="#confirm-del">
                                            <i class="fas fa-trash"></i>
                                        </a>
                                    </div>
                                    </td>
...
@section('modal')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('إضافة وظيفة')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('addjob')}}" id="addJobForm" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    <label>{{Translate('الوظيفة بالعربية')}}</label>
                    <input type="text" name="title_ar" class="form-control">
                </div>
                <div class="form-group">
                    <label>{{Translate('الوظيفة بالإنجليزية')}}</label>
                    <input type="text" name="title_en" class="form-control">
                </div>

                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="addJob()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
    ...
    <div class="modal fade" id="edit-section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{Translate('تعديل وظيفة')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updatejob')}}" id="updateJobForm" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" onclick="updateJob()" class="btn btn-sm btn-success save">{{Translate('حفظ')}}</button>
                    <button class="btn btn-sm btn-danger" data-dismiss="modal">{{Translate('إغلاق')}}</button>
                </div>
            </form>
          </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form action="{{route('deletejob')}}" method="post" class="d-flex align-items-center">
                @csrf
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form action="{{route('deletejobs')}}" method="post" class="d-flex align-items-center">
                @csrf
                <input type="hidden" name="job_ids" id="delete_ids">
    ...
    <script>
        ...
        function addJob() {
            ...
            $.ajax({
                ...
                processData : false,
                contentType : false,
                data        : new FormData($("#addJobForm")[0]),
        ...
        function updateJob() {
            ...
            $.ajax({
                ...
                processData : false,
                contentType : false,
                data        : new FormData($("#updateJobForm")[0]),
                success     : function(msg){
                    if(msg.value == '0'){
                        $('.save').notify(
                            msg.msg, {
